...
ASP.NET Core Web API
   ...
   - AI runtime: TutorAgentService, TutorAgentTools, TutorAgentToolSchemas,
      PromptLoader, OpenAiChatClient, PromptTemplate
...
- AiHintService for agentic hint orchestration
...
2. AiHintService validates the suggested hint level and loads the problem with tags.
3. TutorAgentService loads the system prompt and sends it to OpenAI with four tool definitions.
4. The model decides which tools (if any) to call; TutorAgentTools executes them and feeds results back.
5. The loop continues until the model returns final JSON or the 5-iteration cap is reached.
6. The agent parses the JSON response and falls back to a safe generic hint if the model call fails or returns invalid JSON.
7. AiHintService persists the hint to HintLog, including ToolsUsedJson and ReasoningSummary.

## Key Implementation Notes

- The runtime database provider is SQL Server, not PostgreSQL.
- The active AI path is an **agentic** tutor-agent flow using OpenAI function calling. RAG retrieval, analytics agents, and code-checker agents are not wired into runtime yet.
